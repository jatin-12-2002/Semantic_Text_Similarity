...
class BertSimilarity():
    ...
    def __init__(self, args= None, device='cpu', bert_model_path='bert-base-uncased', batch_size=10, learning_rate = 5e-5, weight_decay=0, additional_features=None):
        if args is not None:
            self.args = vars(args)
        ...
        if not args:
            ...
            self.args['device'] = torch.device("cuda" if torch.cuda.is_available() and self.args['device'] == 'cuda' else "cpu")
